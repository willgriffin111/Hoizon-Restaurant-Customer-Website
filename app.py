    ...
    try:
        if request.method == "POST":
            ...
            if Firstname != None and Seccondname != None and password != None and email != None:
                ...
                if conn != None:    #Checking if connection is None           
                    if conn.is_connected(): #Checking if connection is established
                        ...
                        if dbcursor.rowcount > 0:   #this means there is a user with same name
                            ...
                            return render_template("userRegister.html", error=error, logged_in=session['logged_in'])
                ...
                return render_template("userRegiser.html", error=error, logged_in=session['logged_in'])
        else:            
            return render_template("userRegiser.html", error=error, logged_in=session['logged_in'])        
    except Exception as e:                
        return render_template("userRegiser.html", error=e, logged_in=session['logged_in'])    
    return render_template("userRegiser.html", error=error, logged_in=session['logged_in'])
